...
def find_all_round_keys(main_key_val):
    ...
    for i in range(WORD_LENGTH):
        for j in range(WORD_LENGTH):
            # Converts hex string array to integer
...
def encrypt(text_hex, all_round_keys):
    ...
    for i in range(1, ROUND):
        ...
        current_matrix = add_round_key(
            current_matrix, generate_4x4_matrix(all_round_keys[i]))

    substitute_bytes(current_matrix, True)
    shift_row(current_matrix)
    cipher_text_matrix = add_round_key(
        current_matrix, generate_4x4_matrix(all_round_keys[10]))
...
def decryption(cipher_text, all_round_keys):
    ...
    plain_text_matrix = add_round_key(
        current_matrix, generate_4x4_matrix(all_round_keys[0]))
